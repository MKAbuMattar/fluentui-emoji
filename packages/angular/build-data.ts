 ...
 * tree-shakeable index per style. Data feeds the <fluent-emoji> element.
 */
...
for (const style of STYLES) {
  ...
  for (const {slug, name, attrs, html} of eachIcon(ASSETS, style)) {
    ...
    fs.writeFileSync(
      ...
      `import type {FluentEmojiIcon} from '@fluentui-emoji/angular';\nexport declare const ${name}: FluentEmojiIcon;\nexport default ${name};\n`,
    );
...
console.log(`@fluentui-emoji/angular: generated ${total} icon data modules`);
