 ...
 * tree-shakeable index per style. Data is rendered by src/Emoji.svelte.
 */
...
for (const style of STYLES) {
  ...
  for (const {slug, name, attrs, html} of eachIcon(ASSETS, style)) {
    ...
    fs.writeFileSync(
      ...
      `import type {FluentEmojiData} from '../../../src/index.js';\nexport declare const ${name}: FluentEmojiData;\nexport default ${name};\n`,
    );
...
console.log(`@fluentui-emoji/svelte: generated ${total} icon data modules`);
